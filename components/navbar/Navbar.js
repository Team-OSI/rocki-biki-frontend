...
export default function Navbar({ userEmail, userNickname, userProfileImage, onNicknameUpdate, follow, setFollow }) {
  ...
  return (
      <>
        <nav className="bg-gradient-to-r from-[#7ee70c] to-[#33d7d4] opacity-90 p-4 fixed w-full top-0 left-0 z-50 flex items-center justify-between shadow-md">
          <div className="flex items-center space-x-4">
            <button onClick={toggleFriendsList} className="text-gray-800 hover:text-gray-600 focus:outline-none">
            ...
            <Link href="/lobby" className="text-gray-800 hover:text-gray-600 font-semibold">
              Main
            </Link>
          </div>
          <div className="text-gray-800 hover:text-gray-600 text-lg font-bold absolute left-1/2 transform -translate-x-1/2">
            {title}
          </div>
          <div className="flex items-center space-x-4">
            {userProfileImage && (
